...
{
	std::unique_ptr<FoodVector> food = std::unique_ptr<FoodVector>(new FoodVector);
	size_t i = 0;
	while ((int)food->size() < total_size && i < source.size())
	{
		double calories = source[i]->foodCalories();
		if(calories >= min_calories && calories <= max_calories)
		{
			food->push_back(source[i]);
		}
		i++;
	}
...
{
	// sort by cal / oz
	FoodVector per(foods);
	// vector to be returned
	std::unique_ptr<FoodVector> results(new FoodVector());

	// generate list of calories / oz
	...
	for (auto &i : per)
	{
		double curr = i->weight();
		
		if(currWeight + curr <= total_weight)
		{
			currWeight += curr;
			results->push_back(i);
		}
...
{
	const uint64_t n = foods.size();
	if(n >= 64)
	{
		throw std::domain_error("Bad vector size");
	}

	std::unique_ptr<FoodVector> best(new FoodVector());
  std::unique_ptr<FoodVector> candidate(new FoodVector());
  double candidateCal = 0, candidateOz = 0, bestCal = 0;
	...
	for(uint64_t i = 0; i < pow(2, n); i++)
	{
		...
		sum_food_vector(*candidate, candidateOz, candidateCal);

		if(candidateOz <= total_weight)
		{
			if(best == NULL || candidateCal > bestCal)
			{
				best.swap(candidate);
